    ...
    return (
        <center className="fade-in">
            ...
            {userData.jokes[0] ? userData.jokes.map(joke => (
                joke.joke ?
                    <UserJoke key={joke.id} jokeId={joke._id} category={joke.category} joke={joke.joke} isLightTheme={isLightTheme} userData={userData} setUserData={setUserData}></UserJoke>
                    : <TwoPartUserJoke key={joke.id} jokeId={joke._id} category={joke.category} setup={joke.setup} delivery={joke.delivery} isLightTheme={isLightTheme}></TwoPartUserJoke>
            ))
